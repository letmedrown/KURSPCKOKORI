Shader "Hidden/Locked/.poiyomi/Poiyomi Toon + Lil Fur/bdb2b350fa052eb4da1e2c62a599e1ba"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyFurAOSimple(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				#endif
				poiFragData.baseColor *= (1.0 - 0.0 * mask);
			}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[9.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 0.0), 1.0);
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			PoiVertToGeoLilFur vert(appdata v)
			{
				...
				o.furVector = float4(0,0,1,0.02).xyz + float3(0, 0, 0.001);
				if(0.0) o.furVector = float3(o.furVector.xy + (v.color.xyz * 2.0 - 1.0).xy, o.furVector.z * (v.color.xyz * 2.0 - 1.0).z);
				#if defined(PROP_FURVECTORTEX) || !defined(OPTIMIZER_ENABLED)
				float2 uvMain = v.uv0.xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw;
				float3 furVectorTex = UnpackScaleNormal(_FurVectorTex.SampleLevel(sampler_linear_repeat, uvMain, 0), 1.0);
			...
			float3 applyFurAO(float3 color, float furLayer, float furNoiseMask, float furLayerGradient)
			{
				...
				float furAO = 0.0 * saturate(1.0 - furLayerGradient);
				...
				{
					return color * (saturate(1.0 - furNoiseMask + furNoiseMask * furLayer) * 0.0 * 1.25 + 1.0 - 0.0);
				}
				#endif
			}
			void lilFur(inout PoiFragData poiFragData, PoiGeoToFragLilFur i, float4 mainTex)
			{
				poiFragData.baseColor = mainTex.rgb * float4(1,1,1,1).rgb;
				float furLayer = i.furLayer;
				float furLayerShift = furLayer - furLayer * (0.0) + (0.0);
				float furLayerAbs = abs(furLayerShift);
				float furNoiseMask = 1.0;
				#if defined(PROP_FURNOISEMASK) || !defined(OPTIMIZER_ENABLED)
				furNoiseMask = _FurNoiseMask.Sample(sampler_linear_repeat, i.uv[0].xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw).r;
				#endif
				#if defined(LIL_FUR_PRE)
				...
				#endif
				poiFragData.alpha = mainTex.a * float4(1,1,1,1).a * furAlpha;
			...
			void geom(triangle PoiVertToGeoLilFur input[3], inout TriangleStream<PoiGeoToFragLilFur> outStream)
			{
				...
				furVectors[0] += noise0 * float4(0,0,1,0.02).w * 0.0;
				furVectors[1] += noise1 * float4(0,0,1,0.02).w * 0.0;
				furVectors[2] += noise2 * float4(0,0,1,0.02).w * 0.0;
				#if defined(PROP_FURLENGTHMASK) || !defined(OPTIMIZER_ENABLED)
				float2 lmUV0 = input[0].uv[0].xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw;
				float2 lmUV1 = input[1].uv[0].xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw;
				float2 lmUV2 = input[2].uv[0].xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw;
			...
			float4 frag(PoiGeoToFragLilFur i, uint facing : SV_IsFrontFace) : SV_Target
			{
				UNITY_SETUP_INSTANCE_ID(i);
				UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
				float4 mainTexture = POI2D_SAMPLER_PAN(_MainTex, _MainTex, i.uv[0].xy * float4(1,1,0,0).xy + float4(1,1,0,0).zw, 0);
				PoiSHAr = unity_SHAr;
				PoiSHAg = unity_SHAg;
				PoiSHAb = unity_SHAb;
				PoiSHBr = unity_SHBr;
				...
				float furRimAntiLightValue = lerp(1, dot(furRimInvLighting, float3(1.0/3.0, 1.0/3.0, 1.0/3.0)), 0.5);
				poiFragData.finalColor += i.furLayer * pow((1 - abs(dot(normalize(i.normal), poiCam.viewDir))), 3.0) * furRimAntiLightValue * float4(0,0,0,1).rgb * poiLight.directColor;
				#if defined(POI_PASS_ADD) && !defined(LIL_FUR_PRE)
				if (0.0 == 1)
				{
					poiFragData.finalColor.rgb *= saturate(poiFragData.alpha * 10.0);
				}
				#endif
				#ifndef POI_PASS_LILFUR
				#if !defined(POI_PASS_BASETWO) && !defined(POI_PASS_ADDTWO)
