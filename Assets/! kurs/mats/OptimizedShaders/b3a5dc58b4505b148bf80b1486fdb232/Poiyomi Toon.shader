Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/b3a5dc58b4505b148bf80b1486fdb232"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * (0.22 /*_ShadowStrength*/);
				firstSecondShadowMask *= poiLight.shadowMask * (0.22 /*_ShadowStrength*/);
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float shadowStrength = (0.22 /*_ShadowStrength*/) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
			...
			void applyGlitter(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				for (int glitterLayer = 0; glitterLayer < (0.0 /*_GlitterLayers*/); glitterLayer++)
				{
					float2 st = (poiMesh.uv[(1.0 /*_GlitterUV*/)] + float4(0,0,0,0).xy * _Time.x) * (100000.0 /*_GlitterFrequency*/);
					...
					switch((0.0 /*_GlitterMode*/))
					{
						...
						finalGlitter = lerp(0, (0.0 /*_GlitterMinBrightness*/) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, (0.495 /*_GlitterBias*/)), poiCam.viewDir)), (300.0 /*_GlitterContrast*/)), 0);
					...
					{
						poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * ((2.6 /*_GlitterBrightness*/) + GlitterbrightnessOffset), finalGlitter * glitterTexture.a * glitterMask);
						poiFragData.emission += finalGlitter * glitterColor * max(0, (((2.6 /*_GlitterBrightness*/) + GlitterbrightnessOffset) - 1) * glitterTexture.a) * glitterMask;
					}
					else
					{
						poiFragData.emission += finalGlitter * glitterColor * ((2.6 /*_GlitterBrightness*/) + GlitterbrightnessOffset) * glitterTexture.a * glitterMask;
					}
				}
			}
			#endif
			...
			void MetallicAndSpecularFragDataInit(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float smoothness2 = (0.729 /*_MochieRoughnessMultiplier2*/);
				...
				specularMask *= (0.11 /*_MochieSpecularStrength*/);
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if ((1.0 /*_MochieGSAAEnabled*/))
				{
					...
					if ((1.0 /*_Specular2ndLayer*/) == 1 && (1.91 /*_MochieSpecularStrength2*/) > 0)
					{
				...
				specular = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.1807606,0.1807606,0.1807606,1), (0.0 /*_MochieSpecularTintThemeIndex*/)) * poiLight.occlusion * attenuation;
				if (poiFragData.toggleVertexLights)
				{
					#if defined(VERTEXLIGHT_ON)
					for (int index = 0; index < 4; index++)
					{
						...
						vSpecular += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.1807606,0.1807606,0.1807606,1), (0.0 /*_MochieSpecularTintThemeIndex*/)) * poiLight.occlusion;
					}
					#endif
				}
				if ((1.0 /*_Specular2ndLayer*/) == 1)
				{
					...
					specular2 = poiLight.directColor * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.1807606,0.1807606,0.1807606,1), (0.0 /*_MochieSpecularTintThemeIndex*/)) * poiLight.occlusion * attenuation * (1.91 /*_MochieSpecularStrength2*/);
					if (poiFragData.toggleVertexLights)
					{
						#if defined(VERTEXLIGHT_ON)
						for (int index = 0; index < 4; index++)
						{
							...
							vSpecular2 += poiLight.vColor[index] * specularTerm * fresnelTerm * specularMask * poiThemeColor(poiMods, float4(0.1807606,0.1807606,0.1807606,1), (0.0 /*_MochieSpecularTintThemeIndex*/)) * poiLight.occlusion * (1.91 /*_MochieSpecularStrength2*/);
						}
						#endif
					}
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = min(poiLight.directColor, (0.68 /*_LightingCap*/));
					poiLight.indirectColor = min(poiLight.indirectColor, (0.68 /*_LightingCap*/));
				}
				...
				#endif
				ApplyPoiyomiRimLighting(poiFragData, poiMesh, poiCam, poiLight, poiMods, (1.0 /*_Is_NormalMapToRimLight*/), (0.0 /*_RimLightingInvert*/), (1.0 /*_RimPower*/), (0.75 /*_RimStrength*/), (0.0 /*_RimShadowWidth*/), (0.0 /*_RimShadowToggle*/), (0.456 /*_RimWidth*/), (1.0 /*_RimBlendStrength*/), rimMask, (0.0 /*_RimGlobalMask*/), (2.0 /*_RimGlobalMaskBlendType*/), rimColor, float4(0,0,0,1), (0.0 /*_RimLightColorThemeIndex*/), (0.0 /*_RimHueShiftEnabled*/), (0.0 /*_RimHueShift*/), (0.0 /*_RimHueShiftColorSpace*/), (0.0 /*_RimHueShiftSpeed*/),  (0.0 /*_RimSharpness*/), (0.0 /*_RimShadowMaskRampType*/), (0.0 /*_RimShadowMaskInvert*/), (1.0 /*_RimShadowMaskStrength*/), float4(0,0,0,1), (0.0 /*_RimApplyGlobalMaskIndex*/), (2.0 /*_RimApplyGlobalMaskBlendType*/), (0.0 /*_RimBaseColorMix*/), (1.0 /*_RimBrightness*/), (0.0 /*_RimPoiBlendMode*/), AudioLinkRimWidthBand, AudioLinkRimWidthAdd, AudioLinkRimEmissionBand, AudioLinkRimEmissionAdd, AudioLinkRimBrightnessBand, AudioLinkRimBrightnessAdd, rimBias, (0.0 /*_RimBiasIntensity*/), (0 /*_RimApplyAlpha*/), (1.0 /*_RimApplyAlphaBlend*/));
				#endif
				#endif
				#ifdef POI_RIM2
				#ifdef _RIM2STYLE_POIYOMI
				...
				#endif
				ApplyPoiyomiRimLighting(poiFragData, poiMesh, poiCam, poiLight, poiMods, (1.0 /*_Is_NormalMapToRim2Light*/), (0.0 /*_Rim2LightingInvert*/), (1.0 /*_Rim2Power*/), (0.0 /*_Rim2Strength*/), (0.0 /*_Rim2ShadowWidth*/), (0.0 /*_Rim2ShadowToggle*/), (0.8 /*_Rim2Width*/), (1.0 /*_Rim2BlendStrength*/), rim2Mask, (0.0 /*_Rim2GlobalMask*/), (2.0 /*_Rim2GlobalMaskBlendType*/), rim2Color, float4(0,0,0,1), (0.0 /*_Rim2LightColorThemeIndex*/), (0.0 /*_Rim2HueShiftEnabled*/), (0.0 /*_Rim2HueShift*/), (0.0 /*_Rim2HueShiftColorSpace*/), (0.0 /*_Rim2HueShiftSpeed*/), (0.25 /*_Rim2Sharpness*/), (0.0 /*_Rim2ShadowMaskRampType*/), (0.0 /*_Rim2ShadowMaskInvert*/), (1.0 /*_Rim2ShadowMaskStrength*/), float4(0,0,0,1), (0.0 /*_Rim2ApplyGlobalMaskIndex*/), (2.0 /*_Rim2ApplyGlobalMaskBlendType*/), (0.0 /*_Rim2BaseColorMix*/), (1.0 /*_Rim2Brightness*/), (0.0 /*_RimPoi2BlendMode*/), AudioLinkRim2WidthBand, AudioLinkRim2WidthAdd, AudioLinkRim2EmissionBand, AudioLinkRim2EmissionAdd, AudioLinkRim2BrightnessBand, AudioLinkRim2BrightnessAdd, rim2Bias, (0.0 /*_Rim2BiasIntensity*/), (0 /*_Rim2ApplyAlpha*/), (1.0 /*_Rim2ApplyAlphaBlend*/));
				#endif
				#endif
				#ifdef _SUNDISK_SIMPLE
