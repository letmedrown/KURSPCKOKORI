Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/027ecef95d9a3104da26011e3ca2cf7f"
{
	Properties
	{
		...
		[Normal]_BumpMap ("Normal Map--{reference_properties:[_BumpMapPan, _BumpMapUV, _BumpScale_bodylight, _BumpMapStochastic]}", 2D) = "bump" { }
		...
		[HideInInspector]_BumpScale_bodylight ("Intensity", Range(0, 10)) = 1
		...
		_DecalBlendAlpha1_bodylight ("Alpha", Range(0, 1)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			float _DecalBlendAlpha1_bodylight;
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal1.m_DecalBlendAlpha = _DecalBlendAlpha1_bodylight;
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_BumpMapUV*/)].xy, float4(1,1,0,0)), float4(0,0,0,0), (0.0 /*_BumpMapStochastic*/)), _BumpScale_bodylight);
				#else
